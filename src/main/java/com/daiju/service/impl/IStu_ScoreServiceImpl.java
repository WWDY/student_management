...
public class IStu_ScoreServiceImpl implements IStu_ScoreService {
    ...
    @Override
    @CacheEvict(cacheNames = "stu_score",key = "#courseScore.sId")
    public Integer updateScoreInfo(CourseScore courseScore) {
        return 1;//courseScoreMapper.update(courseScore, new QueryWrapper<CourseScore>().eq("s_id", courseScore.getSId()).eq("c_name", courseScore.getCName()));
    }
}
